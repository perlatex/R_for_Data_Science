...
新型冠状病毒（俗称武汉肺炎）疫情在多国蔓延，本章通过分析[疫情数据](https://github.com/CSSEGISandData/COVID-19)，了解疫情发展，祝愿人类早日会战胜病毒！



```{r, out.width='50%', fig.align='left', fig.cap='电影《传染病》海报', echo=FALSE}
knitr::include_graphics("images/movie_contagion.jpg")
```
...
```{r, out.width='85%', fig.align='left', echo = F}
...
假定你已经下载了数据，比如`time_series_covid19_confirmed_global.csv`， 那么我们可以用`readr::read_csv()`函数直接读取, 关于R语言中文件读取的方法可以参考第 \@ref(readr) 章。
...
d1 %>%
  ...
  scale_x_date(
   ...
   scale_y_continuous(
    breaks = c(0, 10000, 50000, 100000, 200000, 300000),
    labels = scales::comma
    )
```
...
我觉得下面这张图很漂亮，[https://www.ft.com/coronavirus-latest](https://www.ft.com/coronavirus-latest) ，于是打算重复


```{r, out.width='85%', fig.align='left', fig.cap='图片来源www.ft.com', echo=FALSE}
...
d2 %>%
  ...
  geom_shadowtext(
    data = d2_newest, aes(label = paste0(" ", country_region)),
    bg.color = "white"
  ) +
  scale_y_log10(
