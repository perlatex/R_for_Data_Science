...
# str_split(df$y, ":", simplify = TRUE, n = 6)   # or
str_split_fixed(df$y, ":", 6) %>%                # 返回matrix
  as_tibble() %>%
  select(1:4) %>%
...
df %>%
  mutate(
    new = str_split(y, pattern = ":", simplify = FALSE, n = 6) %>% map(., ~.x[1:4]) # 返回 list-column: a list of vectors
  ) %>%
  unnest_wider(col = new, 1:4)  # 利用原来的列名"new"和1:4，构成新列名new11 new22 new33 new44
