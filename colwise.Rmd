...
在dplyr1.0之前，summarise()会把统计结果整理成一行 (one row, one column)，现在可以根据函数返回的结果，整理成多行。
...
dplyr 1.0 之前是需要group_modify()来实现`数据框进，数据框出`
```{r}
mtcars %>% 
  group_by(cyl) %>% 
  group_modify(
...
rowwise()不仅仅是计算行方向的简单统计，而是当rowwise处理列表列时，方才显示出它与`purrr::map`一样的强大。那么，什么是**列表列**？**列表列**指的是数据框的一列是一个列表， 比如
...
事实上，分组建模后，形成**列表列**，这种存储格式，天然地符合**行处理的范式**，因此一开始就转向行方向处理，这里`nest_by()` 类似 `group_by()`
