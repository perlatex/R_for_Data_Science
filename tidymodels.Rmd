...
penguins_pred <- 
  predict(
    wflow_fit, 
    new_data = testing_data %>% select(-bill_length_mm),
    type= "numeric"
  ) %>% 
  dplyr::bind_cols(testing_data %>% select(bill_length_mm))
...
<https://www.tmwr.org/performance.html#regression-metrics>
