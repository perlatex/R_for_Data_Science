...
#### dplyr::mutate()
第三种方法是用`rowwise() + mutate()`，比如，下面为每个岛屿(island) 创建一个企鹅数量相等的随机数向量，简单点说，这个岛屿上企鹅有多少只，那么随机数的个数就有多少个。
```{r}
df %>% 
  select(island) %>% 
  rowwise() %>% 
  mutate(random = list(rnorm(n = str_length(island)))) %>% 
  ungroup()
```

### Unnesting

用unnest()函数可以把**列表列**转换成**常规列**的形式，比如上节中的tb
