...
```{r tidygraph-12, out.width = '50%', fig.align='center'}
ggraph(cor.graph) +
  geom_edge_link() +
  geom_node_point() +
  geom_node_text(
...
```{r tidygraph-19, message=FALSE, warning=FALSE, out.width='50%'}
set.seed(1)
ggraph(cor.graph) +
...
```{r tidygraph-22, message=FALSE, warning=FALSE, out.width='90%'}
set.seed(123)
phone.net %>%
  activate(nodes) %>%
  mutate(centrality = centrality_authority()) %>%
...
```{r tidygraph-24, message=FALSE, warning=FALSE, out.width='90%'}
cluster_mtcars %>%
  ggraph(layout = "graphopt") +
  geom_edge_link(width = 1, colour = "lightgray") +
...
```{r tidygraph-28, message=FALSE, warning=FALSE}
cluster_mtcars %>%
  # Layout
  ggraph(layout = "graphopt") +
  # Edges
