...
用一个简单的案例说明。这里我们对输入的向量每个元素计算平方，可以这样写函数
```{r}
mysquare <- function(x) {
   y <- x^2
   return(y)  
...
::: {.rmdnote}
函数相当于一个机器，这个机器有个功能，对输入进来的参数（这里的参数是一个向量）计算平方值，然后报告结果。
:::
...
现在这台机器有个名字`mysquare`，需要用这个机器的时候，就调用它的名字`mysquare()`。
...
R语言里面，完成一件事情往往有很多中方法，定义函数也不例外。比如`mysquare`可以好几种写法
